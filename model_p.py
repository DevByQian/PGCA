...
class HyperConv(Module):
    ...
    def forward(self, adjacency, adjacency_pv, adjacency_vp, adjacency_uv, adjacency_vu, adjacency_pc, adjacency_cp,
        ...
        for i in range(self.layers):
            ...
            category_embeddings = self.inter_gate(self.a_o_g_c, self.b_o_gc1, cate_emb,
            ...
            # 调用聚合函数
            e_cate_user = self.inter_aggregate_user(
                adjacency_uv, cate_emb
            )
        ...
        return item_embeddings, user_embeddings, category_embeddings
    ...
    def inter_aggregate_user(self, adjacency, cate_emb):
    ...
    def inter_gate_user(self, user_emb, e_item, e_category):
        """
        ...
        """
        # 拼接特征并动态门控
        combined = torch.cat([user_emb, e_item, e_category], dim=-1)
...
class PGCA(Module):
    ...
    def forward(self, session_item, price_seqs, session_len, reversed_sess_item, mask):
        ...
        if not self.hypergraph_skip:
            ...
            item_embeddings_hg, user_embeddings_hg, cate_embeddings_hg = \
        ...
        return item_embeddings_hg, price_embeddings_hg, sess_emb_hgnn, sess_pri_hgnn, item_pri_l, node_loss, sess_loss
...
def forward(model, i, data):
    ...
    item_emb_hg, price_emb_hg, sess_emb_hgnn, sess_pri_hgnn, item_pri_l, node_loss, sess_loss = \
    ...
    scores = scores_interest
    return tar, scores, node_loss, sess_loss
...
def train_test(model, train_data, test_data, lambda1):
    ...
    for i in tqdm(slices, desc="Training", unit="batch"):
        model.zero_grad()
        targets, scores, node_loss, sess_loss = forward(model, i, train_data)  # 新增两个损失项
        # ! 动态权重调整
        ...
        l2_reg = 0.01 * (model.item_loss_weight ** 2 + model.user_loss_weight ** 2 + model.cate_loss_weight ** 2)
        total_batch_loss = main_loss + lambda1 * (node_loss + sess_loss) + l2_reg
        ...
        contrast_loss += (node_loss.item() + sess_loss.item())
    ...
    for i in tqdm(slices, desc="Predicting", unit="batch"):
        tar, scores, _, _ = forward(model, i, test_data)
