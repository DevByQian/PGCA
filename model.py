...
class HyperConv(Module):
    def __init__(self, layers, dataset, emb_size, n_node, n_user, n_price, n_category):
        ...
        # 新增矩阵
        self.W_user = nn.Linear(4 * emb_size, emb_size)
    ...
    def forward(self, adjacency, adjacency_pv, adjacency_vp, adjacency_uv, adjacency_vu, adjacency_pc, adjacency_cp,
        ...
        for i in range(self.layers):
            item_embeddings = self.inter_gate(self.a_o_g_i, self.b_o_gi1, self.b_o_gi2, embedding,
                                              self.get_embedding(adjacency_vp, pri_emb),
                                              self.get_embedding(adjacency_vc, cate_emb)) + self.get_embedding(
                adjacency, embedding) \
            ...
            # 调用聚合函数
            e_price_user, e_cate_user = self.inter_aggregate_user(
                adjacency_uv, pri_emb, cate_emb
            )
            user_embeddings = self.inter_gate_user(user_emb, self.intra_gate(adjacency_uv, self.mat_uv, embedding),
                                                   e_price_user, e_cate_user)
        ...
        return item_embeddings, price_embeddings, user_embeddings, category_embeddings
    ...
    def inter_aggregate_user(self, adjacency, price_emb, cate_emb):
        ...
        return e_price_user, e_cate_user

    def inter_gate_user(self, user_emb, e_item, e_price, e_category):
        """
        ...
        """
        # 拼接特征并动态门控
        combined = torch.cat([user_emb, e_item, e_price, e_category], dim=-1)
...
class PGCA(Module):
    def __init__(self, adjacency, adjacency_pv, adjacency_vp, adjacency_uv, adjacency_vu, adjacency_pc, adjacency_cp,
        ...
        # 添加可视化标志
        self.return_visualization = True  # 用于控制是否返回可视化数据
    ...
    def forward(self, session_item, price_seqs, session_len, reversed_sess_item, mask):
        ...
        if not self.hypergraph_skip:
            ...
            item_embeddings_hg, price_embeddings_hg, user_embeddings_hg, cate_embeddings_hg = \
        ...
        ploss = price_contrast_loss + self.lambda2 * trend_loss
...
def visualize_fusion_comparison(model, test_data, sample_batch=0, save_path=None):
    """Visualize feature distribution comparison before and after fusion"""
    model.eval()
    model.return_visualization = True  # Add flag to return intermediate results

    # Get a batch of data
    slices = test_data.generate_batch(model.batch_size)
    i = slices[sample_batch]

    # Get test data
    tar, session_len, session_item, reversed_sess_item, mask, price_seqs = test_data.get_slice(i)
    session_item = trans_to_cuda(torch.Tensor(session_item).long())
    session_len = trans_to_cuda(torch.Tensor(session_len).long())
    price_seqs = trans_to_cuda(torch.Tensor(price_seqs).long())
    tar = trans_to_cuda(torch.Tensor(tar).long())
    ...
    with torch.no_grad():
        ...
        # Get price-aware module output
        sensitivity, trend_emb = model.price_aware(
            price_emb_hg, price_seqs, model.category_embedding.weight, mask.float()
        )

        # Calculate features before and after fusion
        sess_pri_original = sess_pri_hgnn.clone()  # Original features
        sess_pri_fused = sess_pri_hgnn + sensitivity * trend_emb  # Fused features

    # Convert to numpy
    original_np = sess_pri_original.cpu().numpy()
    fused_np = sess_pri_fused.cpu().numpy()
    sensitivity_np = sensitivity.cpu().numpy()
    trend_emb_np = trend_emb.cpu().numpy()

    # Create first figure: Original feature distribution
    plt.figure(figsize=(10, 10))  # Single figure size
    plt.rcParams['font.family'] = 'Times New Roman'  # Unified Times New Roman
    plt.rcParams['font.size'] = 36  # Base font size (adjustable)

    dims_to_plot = min(10, original_np.shape[1])
    for dim in range(dims_to_plot):
        plt.hist(original_np[:, dim], alpha=0.5, label=f'Dim {dim}', bins=20)
    plt.xlabel('Feature Value')
    plt.ylabel('Frequency')
    plt.legend(
        loc='upper left',
        ncol=2,
        fontsize=30,  # Font size (smaller than base to avoid overlap)
        handlelength=1.5,
        handletextpad=0.2,
        borderpad=0.1,
        labelspacing=0.3,
    ...
    plt.grid(True, alpha=0.3)  # Add gridlines for better readability
    plt.tight_layout()

    # Save first figure as PDF
    if save_path:
        # Generate filename for original feature distribution
        original_path = save_path.replace('.png', '_original.pdf').replace('.pdf', '_original.pdf')
        plt.savefig(original_path, dpi=300, bbox_inches='tight', format='pdf')
        print(f"Original feature distribution plot saved to: {original_path}")
    plt.show()
    plt.close()  # 关闭当前图形，准备下一张

    # Create second figure: Fused feature distribution
    plt.figure(figsize=(10, 10))  # Single figure size
    plt.rcParams['font.family'] = 'Times New Roman'  # Unified Times New Roman
    plt.rcParams['font.size'] = 36  # Base font size (adjustable)

    for dim in range(dims_to_plot):
        plt.hist(fused_np[:, dim], alpha=0.5, label=f'Dim {dim}', bins=20)
    plt.xlabel('Feature Value')
    plt.ylabel('Frequency')
    plt.legend(
        loc='upper left',
    ...
    # Save second figure as PDF
    if save_path:
        # Generate filename for fused feature distribution
        fused_path = save_path.replace('.png', '_fused.pdf').replace('.pdf', '_fused.pdf')
        plt.savefig(fused_path, dpi=300, bbox_inches='tight', format='pdf')
        print(f"Fused feature distribution plot saved to: {fused_path}")
    plt.show()
    plt.close()  # 关闭图形

    return {
        'original_features': original_np,
        'fused_features': fused_np,
        'sensitivity': sensitivity_np,
        'trend_embeddings': trend_emb_np,
        'change_stats': {
            'mean_change': np.mean(np.abs(fused_np - original_np)),
            'max_change': np.max(np.abs(fused_np - original_np)),
            'change_ratio': np.mean(np.abs((fused_np - original_np) / (np.abs(original_np) + 1e-8)))
        }
    }