...
def visualize_fusion_comparison(model, test_data, sample_batch=0, save_path=None):
    ...
    # 创建第一张图：原始特征分布
    plt.figure(figsize=(10, 10))  # 单图尺寸
    plt.rcParams['font.family'] = 'Times New Roman'  # 统一使用Times New Roman
    plt.rcParams['font.size'] = 36  # 基础字体大小（可调整）
    ...
    if save_path:
        ...
        print(f"原始特征分布图已保存到: {original_path}")
    plt.show()
    ...
    if save_path:
        ...
        print(f"融合后特征分布图已保存到: {fused_path}")
    plt.show()
    plt.close()  # 关闭图形
